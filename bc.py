            ...
            if (len(tokens) == 0 or tokens[-1].type == 'opr' or tokens[-1].value == "(") and exp[i] == '-':
                ...
                if exp[j].isdigit():
                    ...
                    if var in variables:
                        value = variables[var]
                    #print(var)
                    tokens.append(Token('var', value * -1))
                elif exp[j].isalpha():
                    var = ''
                    while j < len(exp) and exp[j].isalpha() :
                        var += exp[j]
...
result = evaluate_expression(tokens, variables)
print(result)


variables = {'x': 5, 'y': 0}
op_equals(variables['x'], '/=', variables['y'])
print(variables['x']) 
